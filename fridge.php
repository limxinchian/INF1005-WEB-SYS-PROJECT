...
<body>
    ...
    <div class="container">
        ...
        <form action="actions/fridge-update.php" method="POST">
            <div class="card d-flex flex-lg-row flex-col gap-3 p-4 mb-4">
                <input
                    type="text"
                    id="ingredientSearch"
                    class="flex-grow-1"
            ...
            <?php if (!empty($allIngredients)): ?>
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-3">
                    <?php foreach ($allIngredients as $ingredient): ?>
                        ...
                        ?>
                        <div class="col">
                            <div class="card p-3 h-100 searchable-item <?= $checked ? 'selected' : '' ?>"
                                ...
                                <label class="ingredient-label">
                                    ...
                                    <span class="ingredient-info">
                                        <span class="ingredient-name"><?= htmlspecialchars(ucwords(strtolower($ingredient['ingredient_name']))) ?></span>
                                        <span class="fs-small">Ingredient ID: <?= $id ?></span>
                                        <span class="fs-small"><?= $allergenText !== '' ? 'Allergens: ' . htmlspecialchars($allergenText) : 'No allergen tags recorded' ?></span>
                                    </span>
                                </label>
                            </div>
                        </div>
