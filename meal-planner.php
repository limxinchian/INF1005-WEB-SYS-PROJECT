...
<div class="container">
    ...
    <div class="d-flex flex-column flex-lg-row gap-4 mt-3">
        ...
        <div class="card p-3 flex-grow-1 d-flex flex-column">
            ...
            <form action="actions/meal-plan-save.php" method="POST" class="d-flex flex-column flex-grow-1 mt-3">
            ...
            <?php if (!empty($allPlans)): ?>
                ...
                <div class="d-flex flex-column align-items-center gap-2">
                    <?php if (count($allPlans) > 1): ?>
                        <button class="btn btn-sm btn-outline-none w-100" type="button" id="planCarouselPrev">&#9650;</button>
                    <?php endif; ?>
                    ...
                    <?php if (count($allPlans) > 1): ?>
                        <button class="btn btn-sm btn-outline-none w-100" type="button" id="planCarouselNext">&#9660;</button>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                <div class="empty-state">You have not created any meal plans yet.</div>
            <?php endif; ?>
        </div>
    </div>
                            ...
                            <?php foreach ($slots as $slot): ?>
                                ...
                                <td>
                                    <div class="meal-cell" data-day="<?= htmlspecialchars($day) ?>" data-slot="<?= htmlspecialchars($slot) ?>">
                                        <input type="hidden" class="recipe-id-input" name="entries[<?= htmlspecialchars($day) ?>][<?= htmlspecialchars($slot) ?>][recipe_id]" value="<?= $entry ? (int)$entry['recipe_id'] : '' ?>">
                                        <input type="hidden" class="recipe-title-input" value="<?= $entry ? htmlspecialchars($entry['title']) : '' ?>">
                                        <div class="recipe-display">
                                            <?= $entry ? '<strong>' . htmlspecialchars($entry['title']) . '</strong>' : '<div class="meal-placeholder">No recipe selected</div>' ?>
                                        </div>
                                        <div class="cell-actions d-flex gap-1 mt-1">
    ...
    <?php else: ?>
        <div class="empty-state mt-3">Create a meal plan first to start filling your weekly planner.</div>
    <?php endif; ?>
